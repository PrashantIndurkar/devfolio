...
export const Tooltip = ({
  ...
  useEffect(() => {
    if (isVisible && contentRef.current) {
      // Use offsetHeight for more accurate height calculation
      const updateHeight = () => {
  ...
  const calculatePosition = (mouseX: number, mouseY: number) => {
    ...
    const tooltipWidth = Math.max(240, tooltip.offsetWidth || 1400); // min-w-[15rem] = 240px, but allow larger
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="pointer-events-none absolute z-50 max-w-full min-w-full overflow-hidden rounded-md border border-transparent bg-white shadow-sm ring-1 shadow-black/5 ring-black/5 dark:bg-neutral-900 dark:shadow-white/10 dark:ring-white/5"
            style={{
              top: position.y,
              left: position.x,
