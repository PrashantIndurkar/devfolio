"use client";

import { ArrowUpRightIcon } from "lucide-react";
import Image from "next/image";
import { useTheme } from "next-themes";
import React from "react";

import type { SocialLink } from "@/features/profile/types/social-links";
import { cn } from "@/lib/utils";

const BRAND_COLORS_LIGHT: Record<string, string> = {
  LinkedIn: "#0A66C2",
  GitHub: "#181717",
  "X (Formerly Twitter)": "#000000",
  Instagram: "#E4405F",
  Email: "#EA4335",
  Resume: "#0061FF",
};

const BRAND_COLORS_DARK: Record<string, string> = {
  LinkedIn: "#0A66C2",
  GitHub: "#FFFFFF",
  "X (Formerly Twitter)": "#FFFFFF",
  Instagram: "#E4405F",
  Email: "#EA4335",
  Resume: "#0061FF",
};

export function SocialLinkItem({
  icon,
  title,
  description,
  href,
  rounded,
}: SocialLink) {
  const { resolvedTheme } = useTheme();
  const isDark = resolvedTheme === "dark";
  const isIconComponent = typeof icon !== "string";
  const IconComponent = isIconComponent ? icon : null;
  const brandColors = isDark ? BRAND_COLORS_DARK : BRAND_COLORS_LIGHT;
  const brandColor = brandColors[title] || undefined;

  return (
    <a
      className={cn(
        "group/link flex cursor-pointer items-center gap-4 rounded-2xl p-4 pr-2 transition-colors select-none",
        "max-sm:screen-line-before max-sm:screen-line-after",
        "sm:nth-[2n+1]:screen-line-before sm:nth-[2n+1]:screen-line-after"
      )}
      href={href}
      target="_blank"
      rel="noopener"
    >
      <div className="relative flex size-12 shrink-0 items-center justify-center rounded-xl bg-background">
        {isIconComponent && IconComponent ? (
          <span
            className={cn(
              "flex items-center justify-center",
              rounded && "rounded-xl"
            )}
            style={brandColor ? { color: brandColor } : undefined}
          >
            <IconComponent className="size-12" />
          </span>
        ) : (
          <Image
            className={cn(rounded && "rounded-xl")}
            src={icon as string}
            alt={title}
            width={48}
            height={48}
            quality={100}
            unoptimized
          />
        )}
      </div>

      <div className="flex-1">
        <h3 className="flex items-center font-medium underline-offset-4 group-hover/link:underline">
          {title}
        </h3>

        {description && (
          <p className="text-sm text-muted-foreground">{description}</p>
        )}
      </div>

      <ArrowUpRightIcon className="size-4 text-muted-foreground" />
    </a>
  );
}
